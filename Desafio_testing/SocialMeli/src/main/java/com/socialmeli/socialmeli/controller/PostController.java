...
public class PostController {
    ...
    public ResponseEntity<?> followedSellersPost(@PathVariable("userId") int user_id,@RequestParam(required = false) String order){

        return new ResponseEntity(userAndPostService.followedSellersPost(user_id,order),HttpStatus.OK);
    }

    @PostMapping("/promo-post")
    public ResponseEntity<?> newPostWithDiscount(@Valid @RequestBody PostDTO postDTO){
        return new ResponseEntity(userAndPostService.newPost(postDTO), HttpStatus.OK);
    }
