...
public class PostsController {
    ...
    //US 0005
    @PostMapping("/post")
    public void loadPost(@Valid @RequestBody PostDTO newPost) throws PostAlreadyExistException,
            InvalidPromoException, UserNotFoundException {
        service.pushPost(newPost);
    }

    //US 0006
    @GetMapping("/followed/{userId}/list")
    public PostsResponseDTO getFollowedPostList(@PathVariable Integer userId,
    ...
    public void loadPromoPost(@Valid @RequestBody PostDTO newPost) throws PostAlreadyExistException,
            InvalidPromoException, UserNotFoundException {
        double epsilon = 1e-4;
        if (!newPost.isHasPromo() || newPost.getDiscount() < epsilon ||
                newPost.getDiscount() > 100 + epsilon) {
    ...
    public CountPromosResponseDTO getPromoCount(@PathVariable Integer userId)
            throws UserNotFoundException {
