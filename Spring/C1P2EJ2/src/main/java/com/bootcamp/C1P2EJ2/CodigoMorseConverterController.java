...
@RestController
public class CodigoMorseConverterController {
    @GetMapping("/{morse}")
    public String convertMorseToString (@PathVariable String morse){
        String[] palabrasEnMorse = morse.split("   ");
        List<String[]> letrasEnMorse = new ArrayList<String[]>();
        for (int i = 0; i < palabrasEnMorse.length;i++){
            letrasEnMorse.add(palabrasEnMorse[i].split(" "));
        }
        String palabra = "";
        for (int j = 0; j < letrasEnMorse.size(); j++){
            for (int k = 0; k < letrasEnMorse.get(j).length; k++){
                palabra += (morseAAscii(letrasEnMorse.get(j)[k]));
            }
            palabra += " ";
        }
        return palabra;
