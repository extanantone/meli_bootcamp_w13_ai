...
public class UserService implements UserServiceI {
    ...
    public ResponseEntity<String> followUser(int userId, int userIdToFollow) {
        ...
        if (userId != userIdToFollow && isUserIdValid(userId) && isUserIdValid(userIdToFollow)) {
            ...
            if (!followers.contains(userId) && !followedUsers.contains(userIdToFollow)) {
                ...
                throw new BadRequestException("User with id " + userId + " already follows user with id " + userIdToFollow);
            }
    ...
    public ResponseEntity<String> unfollowUser(int userId, int userIdToUnfollow) {
        ...
        if (userId != userIdToUnfollow && isUserIdValid(userId) && isUserIdValid(userIdToUnfollow)) {
            ...
            if (followers.contains(userId) && followedUsers.contains(userIdToUnfollow)) {
                ...
                throw new BadRequestException("User with id " + userId + " didn't follow the user with id " + userIdToUnfollow);
            }
    ...
    @Override
    public ResponseEntity<FollowersCountDTO> getFollowersCount(int userId) {
        HashMap<Integer, User> usersList = repository.getUsers();
        if (isUserIdValid(userId)) {
            ...
            if (user.getIsSeller()) {
                ...
                FollowersCountDTO res = new FollowersCountDTO(userId, user.getUserName(), followersCount);
    ...
    @Override
    public ResponseEntity<FollowersDTO> getSellerFollowersList(int userId, String order) {
        HashMap<Integer, User> usersList = repository.getUsers();
        User user = usersList.get(userId);
    ...
    @Override
    public ResponseEntity<FollowedUsersDTO> getFollowedSellersList(int userId, String order) {
        HashMap<Integer, User> usersList = repository.getUsers();
        User user = usersList.get(userId);
        ...
        if (isUserIdValid(userId)) {
            ...
            for (Integer id : followedUsers) {
                ...
                if (followedUser.getIsSeller()) {
                    UserDTO followerDTO = new UserDTO(followedUser.getId(), followedUser.getUserName());
    ...
    private List<PostsListDTO> createPostsListResponse(int userId, String order) {
        HashMap<Integer, User> usersList = repository.getUsers();
        User user = usersList.get(userId);
    ...
    @Override
    public ResponseEntity<List<PostsListDTO>> getRecentFollowedSellersPosts(int userId) {
        if (isUserIdValid(userId)) {
            return new ResponseEntity<>(createPostsListResponse(userId, "date_desc"), HttpStatus.OK);
        } else {
            throw new NotFoundException("The user does not exist");
        }
    }
    ...
    @Override
    public ResponseEntity<List<PostsListDTO>> getRecentFollowedSellersPostsReverse(int userId) {
        if (isUserIdValid(userId)) {
            return new ResponseEntity<>(createPostsListResponse(userId, "date_asc"), HttpStatus.OK);
        } else {
            throw new NotFoundException("The user does not exist");
        }
    }
    ...
    public ResponseEntity<String> createPromoPost(PromoPostDTO newPromoPost) {
        HashMap<Integer, User> usersList = repository.getUsers();
        int userId = newPromoPost.getUserId();
    ...
    @Override
    public ResponseEntity<PromoProductsCountDTO> getPromoProductsCount(int userId) {
        HashMap<Integer, User> usersList = repository.getUsers();
        if (isUserIdValid(userId)) {
            User user = usersList.get(userId);
            if (user.getIsSeller()) {
                int promoProductsCount = user.getPromoPosts().size();

                PromoProductsCountDTO res = new PromoProductsCountDTO(userId, user.getUserName(), promoProductsCount);
                return new ResponseEntity<>(res, HttpStatus.OK);
            } else {
                throw new BadRequestException("The user is not a seller");
            }
    ...
    @Override
    public ResponseEntity<PromoPostsListDTO> getPromoProductsList(int userId) {
        HashMap<Integer, User> usersList = repository.getUsers();
